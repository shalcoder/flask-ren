    ...
    if request.method == 'POST':
        ...
        if not success:
            ...
            if error_status == 'NOT_FOUND':
                ...
                user_message = "No routes could be found for the destination."
            ...
            <body>
                ...
                <script>
                    ...
                    function updateUIForStep(stepIdx) {
                        fetch(`/current_step_set/${stepIdx}`)
                            .then(response => response.json())
                            .then(data => {
                                if (!data.error) {
                                    document.getElementById('currentStepNum').textContent = data.step_index + 1;
        ...
        total_distance=current_route['total_distance_text'],
        total_duration=current_route['total_duration_text'])
...
def update_location():
    ...
    if current_route['steps']:
        ...
        THRESHOLD_METERS_TO_ADVANCE = 15
